...
class Collider:
    ...
    def get_min(self, transform: Transform):
        return transform.position - transform.scale * self.size / 2.0
    
    def get_max(self, transform: Transform):
        return transform.position + transform.scale * self.size / 2.0

    def clip_x(self, transform: Transform, other_transform: Transform, collider, velocity: float):
    ...
    def clip_y(self, transform: Transform, other_transform: Transform, collider, velocity: float):
        min_a = self.get_min(transform)
        max_a = self.get_max(transform)
    ...
    def clip_z(self, transform: Transform, other_transform: Transform, collider, velocity: float):
        min_a = self.get_min(transform)
        max_a = self.get_max(transform)
